...
--[[
 ...
 - Select direct children of "node-834" (id: 834, name: node-834, parent: 770, path: 1/513/769/770/834/, lnum: 166300, rnum: 178800, level 5).
 -
...
dofile(pathtest .. "03_nested-insert.lua")
...
FROM `animals` AS `child`
  INNER JOIN `animals` AS `parent`
    ON `child`.`left` > `parent`.`left` AND `child`.`left` < `parent`.`right`
...
WHERE `parent`.`id` = 834 AND `intermediate`.`id` IS NULL
]])
end
